...
function StorefrontHome() {
  ...
  const sections: Record<string, ReactNode> = {
    ...
    products: (
      <section className="pb-10">
        <div className="mb-6 rounded-xl border p-3 sm:p-4" style={{ borderColor: "var(--sf-border)", background: "var(--sf-surface)" }}>
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="h-10 rounded-lg border bg-transparent px-3 text-sm">
              <option value="featured">Featured</option><option value="price-low">Price: Low to High</option><option value="price-high">Price: High to Low</option><option value="rating">Highest Rated</option><option value="name">Name: A-Z</option>
            </select>
          </div>
        </div>
        {isLoading ? (

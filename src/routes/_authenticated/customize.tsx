...
function ThemeSettingsPanel({ isLifetime, draft, draftTheme, currency, userId, onTheme, onCurrency, onUpdate }: { isLifetime: boolean; draft: ThemeSettings; draftTheme: string; currency: string; userId: string; onTheme: (theme: string) => void; onCurrency: (currency: string) => void; onUpdate: (patch: Partial<ThemeSettings>) => void }) {
  return <div className="max-h-[700px] overflow-y-auto p-4"><div className="space-y-6">
    ...
    <div className="border-t pt-5"><p className="text-sm font-semibold">Store layout</p><p className="mt-1 text-xs text-muted-foreground">The selected theme starts with its own layout. You can override it here.</p><select value={draft.layoutOverride || ""} onChange={(e) => onUpdate({ layoutOverride: (e.target.value || undefined) as ThemeSettings["layoutOverride"] })} className="mt-3 h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="">Use {getTheme(draftTheme).name} default</option>{(["grid","editorial","list","showcase","lookbook","bento","split","catalog","minimal","immersive","masonry"] as const).map((layout) => <option key={layout} value={layout}>{layout.charAt(0).toUpperCase() + layout.slice(1)}</option>)}</select></div>
